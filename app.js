...
function updateProduct() {
  ...
  if (state.mode === "pallets") {
    ...
    $("saving").textContent =
      `Zamawiasz ${state.qty} ${pluralizePallets(state.qty)}. Każda paleta to 65 worków.`;
  } else {
    $("qtySlider").max = 64;
    ...
    if (next) {
      ...
      if (next.isPallet) {
        $("saving").textContent =
          `Dodaj jeszcze ${diff} ${pluralizeBags(diff)} do pełnej palety, aby uzyskać najlepszą cenę (36,15 PLN / worek).`;
      } else {
        const extra = Math.max(0, next.price - c.total);
        $("saving").textContent =
          `Dodaj ${diff} ${pluralizeBags(diff)}, aby wejść w kolejny próg. Szacunkowo +${money(extra)}.`;
      }
    }
  }
  ...
  if (state.mode === "bags") {
    $("tierLabel").textContent =
      `Wybrano: ${state.qty} ${pluralizeBags(state.qty)}`;
  } else {
...
function renderCart() {
  ...
  if (!state.cartQty) {
    el.innerHTML =
      '<p class="empty">Nie wybrano pakietu. Zaznacz ilość powyżej i dodaj do zamówienia.</p>';
    $("cartTotal").textContent = "0 PLN";
    return;
  }
  const c = calc(state.cartQty, state.cartMode);
  const title =
    state.cartMode === "pallets"
...
$("sendOrder").onclick = async () => {
  ...
  if (digits.length === 9) {
    ...
    customer.phone =
      "+48 " + digits.substring(2).replace(/(\d{3})(?=\d)/g, "$1 ");
  ...
  const items =
    state.cartMode === "pallets"
      ? [
          {
            title: "Pellet drzewny — Paleta (65 worków)",
            quantity: state.cartQty,
            unit_weight_kg: 975,
          },
        ]
      : [
          {
            title: "Pellet drzewny — worek 15 kg",
            quantity: state.cartQty,
            unit_weight_kg: 15,
          },
        ];
  ...
  try {
    ...
    try {
      ...
      throw new Error(
        `Parse error (${response.status}): ${text.substring(0, 50)}...`,
      );
    }
